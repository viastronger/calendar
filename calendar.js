(function ($) {
  ...
  function dateClick(obj) {
    var index = $(obj).index()
    if (calendarDate.oddOrEven % 2 == 0) {
      $(obj).siblings().each(function () {
        $(this).removeClass('ht-rili-td-active');
      });
      calendarDate.clickFirst = index
      calendarDate.clickTwo = null
      $(obj).addClass('ht-rili-td-active');
    } else {
      calendarDate.clickTwo = index
    }
    calendarDate.oddOrEven += 1
    if (calendarDate.oddOrEven > 2) {
      calendarDate.oddOrEven = 1
    }
    if (calendarDate.clickTwo) {
      if (calendarDate.clickTwo > calendarDate.clickFirst) {
        for (var i = calendarDate.clickFirst; i < calendarDate.clickTwo + 1; i++) {
          $('.ht-rili-td').eq(i).addClass('ht-rili-td-active');
        }
      } else {
        for (var i = calendarDate.clickTwo; i < calendarDate.clickFirst + 1; i++) {
          $('.ht-rili-td').eq(i).addClass('ht-rili-td-active');
        }
      }
    }
